...
*/
/**
* @version $Id: admin.users.php 421 2005-10-09 18:18:00Z stingrey $
* @package Joomla
* @subpackage Users
...
function removeUsers( $cid, $option ) {
	...
	//Only Administrator and Superadministrator can delete users.
	if ( ( $my->gid != 24 ) && ( $my->gid != 25 ) ){ 
		echo "<script> alert('Only Administrators and Super Administrators can delete users'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			$group_id = intval($groups[0]);
			if ( $id == $my->id ){
 				$msg = "You cannot delete Yourself!";
 			} else if ( ( $group_id == 25 ) && ( $my->gid != 25 ) ) {
				$msg = "You cannot delete a Super Administrator";
 			} else if ( ( $group_id == 24 ) && ( $my->gid != 25 ) ){
 				$msg = "You cannot delete another `Administrator` only `Super Administrators` have this power";
			} else {
				$obj->delete( $id );
				$msg = $obj->getError();
