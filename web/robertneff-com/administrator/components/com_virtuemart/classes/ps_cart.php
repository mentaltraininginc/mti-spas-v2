...
/**
*
* @version $Id: ps_cart.php,v 1.12 2005/11/04 15:16:48 soeren_nb Exp $
* @package VirtueMart
* @subpackage classes
...
class ps_cart {
	...
	function add(&$d) {
		...
		################## JBRIDGE START ###########
		$ptp = ptp_main_cart_add(array('product_id'=>$product_id, 'quantity'=>$quantity, 'description'=>$d['description']));
		$quantity = $ptp['quantity'];
		if (!$quantity)
			return false;
		if ($ptp['new_user_level'])
			$_SESSION['cart'][$i]["new_user_level"] = $ptp['new_user_level'];
		################## JBRIDGE END ###########
		...
		if ($quantity == 0) {
			...
			for ($i=0;$i<$_SESSION['cart']["idx"];$i++) {
				...
				($_SESSION['cart'][$i]["description"] == $d["description"] )
					...
					################## PARAMETER HACK START ###########
					$ptp = ptp_main_cart_update(array('product_id'=>$product_id,'quantity'=>$d['quantity'], 'description'=>$d['description']));
					$quantity = $ptp['quantity'];
					################## PARAMETER HACK END ###########
