...
/**
*
* @version $Id: ps_checkout.php,v 1.22 2005/11/24 06:25:40 soeren_nb Exp $
* @package VirtueMart
* @subpackage classes
...
----------------------------' 
		...
		################## JBRIDGE START ###########
        /**
        * Setup order and item default status if PTP order_status field set
        * otherwise use default 'P' status
        */
		$_SESSION['is_recur'] = '';
        $_SESSION['new_user_level'] = '';
		if ($payment_class == 'ps_paypal') {
			$this->aryProduct['order_default_status'] = 'P';
			for($i = 0; $i < $cart["idx"]; $i++) {
    			$ptp = ptp_main_cart_checkout(array('product_id'=>$cart[$i]['product_id'], 'quantity'=>$cart[$i]['quantity']));
	    		if ($cart[$i]['new_user_level'])
		    		$_SESSION['new_user_level'] = $cart[$i]['new_user_level'];
            }
        }
		else {
			$aryStatuses = array(
			...
			for($i = 0; $i < $cart["idx"]; $i++) {
                // get individual item cost for payment modules that support it
				$testPrice = $ps_product->get_adjusted_attribute_price($cart[$i]["product_id"], $cart[$i]["description"]);
				$testProductPrice = $testPrice["product_price"];
				$this->aryProduct[$i]['cost'] = $testProductPrice * $cart[$i]["quantity"];
                // set product status if product type parameter 'order_status' set
				if ($orderStatus = get_param_order_status($cart[$i]['product_id'])) {
					$this->aryProduct[$i]['order_status'] = $aryStatuses[$orderStatus];
					$aryStatus[$orderStatus] = $aryStatuses[$orderStatus];
				}
				else
					$this->aryProduct[$i]['order_status'] = 'P';
			}
            // set default order status
			if (count($this->aryProduct) == 1) {
				$this->aryProduct['order_default_status'] = $this->aryProduct[0]['order_status'];
			}
			else
				if (count($aryStatus) > 1)
					$this->aryProduct['order_default_status'] = 'P';
				else
					$this->aryProduct['order_default_status'] = $aryStatuses[$orderStatus];
			if (!$this->aryProduct['order_default_status'])
				$this->aryProduct['order_default_status'] = 'P';
		}
		################## JBRIDGE END ###########
		...
		if (file_exists(CLASSPATH . "payment/$payment_class.php") ) {
			...
			################## JBRIDGE START ###########
            // pass $this->aryProduct array to payment module
			$_PAYMENT->aryProduct = array();
			$_PAYMENT->aryProduct = $this->aryProduct;
			################## JBRIDGE END ###########
		...
		################## JBRIDGE START ###########
        // set order status
		$q .= "'".$this->aryProduct['order_default_status']."', '";
		################## JBRIDGE END ###########
		$q .= $timestamp . "', '";
		$q .= $timestamp. "', '";
    ...
    */
		...
		$_SESSION['is_recur'] = '';
		$_SESSION['myTotalRecur'] = '';
		$_SESSION['myRegisterFee'] = '';
		$_SESSION['mySKU'] = '';
		################## JBRIDGE END ###########
		for($i = 0; $i < $cart["idx"]; $i++) {
			...
			################## JBRIDGE START ###########
            // if recurring payment and a register fee is set, set session variables
            // this is required for paypal type transactions
			$ptp = ptp_main_cart_checkout(array('product_id'=>$cart[$i]['product_id'], 'quantity'=>$cart[$i]['quantity'], 'description'=>$cart[$i]['description']));
			if ($ptp['recur']) {
				$_SESSION['is_recur'] = true;
				$_SESSION['mySKU'] = addslashes($dboi->f("product_sku"));
				$_SESSION['myTotalRecur'] += $product_final_price;
				if ($ptp['register_fee'])
					$_SESSION['myRegisterFee'] += $ptp['register_fee'];
			}
			################## JBRIDGE END ###########
			...
			$q .= $product_currency . "', ";
			################## JBRIDGE START ###########
            // if paypal type, set item status to pending
			if ($payment_class == 'ps_paypal')
				$q .= "'P','";
			else
				$q .= "'".$this->aryProduct[$i]["order_status"]."','";
			################## JBRIDGE END ###########
	...
	***************************************************************************/
	function calc_order_subtotal($d) {
		...
		################## JBRIDGE START ###########
        // initialize aryProduct class array
        // this holds order and product processing data used in this file
		$this->aryProduct = array();
		################## JBRIDGE END ###########
		for($i = 0; $i < $cart["idx"]; $i++) {
			$price = $ps_product->get_adjusted_attribute_price($cart[$i]["product_id"], $cart[$i]["description"]);
			$product_price = $price["product_price"];
			if( $auth["show_price_including_tax"] == 1 ) {
				...
				################## JBRIDGE START ###########
                // set product cost
				$this->aryProduct[$i]["cost"] = $product_price;
				################## JBRIDGE END ###########
	...
	***************************************************************************/
	function email_receipt($order_id) {
		...
		// ******************************
		// Shopper Header
		$shopper_header = "<a title=\"".$VM_LANG->_PHPSHOP_CHECKOUT_EMAIL_SHOPPER_HEADER5."\"
			href=\"$shopper_order_link\">" .
			$VM_LANG->_PHPSHOP_CHECKOUT_EMAIL_SHOPPER_HEADER1B."</a> \n";
	...
	**************************************************************************/
	function final_info() {
		...
		<table border="0" cellpadding="3" cellspacing="2" width="100%">
			<tr>
				<th colspan="3" style="padding-left:15px" align="left" bgcolor="#C2D6F1">
					<strong>
			...
			<tr>
				<td bgcolor="#E9F0FA" height="175px" valign="top">
					<table cellpadding="3px" cellspacing="0" width="100%">
						<tr>
							...
							<td width="50%" align="left">
								<strong>
									Type <?php echo $_SESSION['ccdata']['creditcard_code']; ?><br />
									Number <?php echo ps_checkout::asterisk_pad($_SESSION['ccdata']['order_payment_number'], 4)?><br />
									Expires <?php echo $_SESSION['ccdata']['order_payment_expire_month'] . '/' . $_SESSION['ccdata']['order_payment_expire_year']; ?>
								</strong>
							</td>
						</tr>	
						<tr>
							...
							<td width="50%" align="left">
								<strong>
									<?php echo $_SESSION['ccdata']['order_payment_name']; ?>
								</strong>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td width="50%" align="left">
								<strong>
									<?php echo $db->f("country").' / '. $db->f("zip"); ?>
								</strong>
							</td>
						</tr>
					</table>
