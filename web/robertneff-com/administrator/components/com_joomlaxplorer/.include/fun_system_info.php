...
------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------
Author: joomla Dev Team
	  Soeren

Comment:
	New for joomlaXplorer
	Replaces the missing System Info Panel in joomla >= 4.5.2
	Have Fun...
------------------------------------------------------------------------------*/
//------------------------------------------------------------------------------
...
function get_php_setting($val) {
	$r =  (ini_get($val) == '1' ? 1 : 0);
	return $r ? '<span style="color: green;">' . $GLOBALS['messages']['sionoff'][0] . '</span>' : '<span style="color: red;">' . $GLOBALS['messages']['sionoff'][1] . '</span>';
}
...
function system_info( $version, $option ) {
	...
	$tabs = new mosTabs(0);
	?><br /><div class="sectionname" align="left">
	<img src="images/systeminfo.png" border="0" align="center" alt="<?php echo $GLOBALS['messages']['simamsysinfo']; ?>" />&nbsp;<?php echo $GLOBALS['messages']['simamsysinfo']; ?></div>
	<br />
	<?php
	...
	<tr>
		<td style="font-weight:bold;">
			<?php echo $GLOBALS['messages']['sijoomlaversion']; ?>:
		</td>
		<td>
		<?php echo $version; ?>
		</td>
	...
	<tr>
		...
		<td>
			...
			<?php
			$query = "SELECT name FROM #__joomlats"
	...
	<table class="adminform">
	  ...
	  <tr>
		...
		<?php
		  ...
		  mosHTML::writableCell( 'joomlats' );
		  mosHTML::writableCell( 'joomlats/content' );
		  mosHTML::writableCell( 'joomlats/search' );
