...
class ff_importPackage extends ff_xmlPackage
{
	...
	function import($filename)
	{
		...
		if (!$ok) {
			...
			if ($errmode=='log') {
				...
				$errors[] = _FACILEFORMS_INSTALLER.': '.$this->error;
			} // if
		} else {
			// success case
	...
	function dropPackage($id)
	{
		...
		$this->oldscripts = _ff_select(
			"select id, name from #__facileforms_scripts where package = binary '$id'"
		);
		$this->oldpieces = _ff_select(
			"select id, name from #__facileforms_pieces where package = binary '$id'"
		);
	...
	function emitScript()
	{
		...
		$row->name        = $this->getText(1, 'name', _FACILEFORMS_INSTALLER_UNKNOWN);
		$row->title       = $this->getText(1, 'title', _FACILEFORMS_INSTALLER_UNKNOWN);
				...
				if ($id == NULL) {
					...
					if ($id == NULL) {
						...
						if ($id == NULL) {
							...
							$cond = 2;
							$code = '// '.$tag.' '.$idx.'/'.$name.' '._FACILEFORMS_INSTALLER_TAGNOTFOUND;
							$nm   = $this->getText($pid, 'name');
							$this->warnings[] = $nm.': '.$tag.' '.$idx.'/'.$name.' '._FACILEFORMS_INSTALLER_TAGNOTFOUND;
						} // if
					} // if
				} // if
				break;
	...
	function emitElement()
	{
		...
		$row->title     = $this->getText(2, 'title', _FACILEFORMS_INSTALLER_UNKNOWN);
		$row->name      = $this->getText(2, 'name', _FACILEFORMS_INSTALLER_UNKNOWN);
