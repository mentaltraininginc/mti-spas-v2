...
class facileFormsConfig extends facileFormsConf {
	...
	function save($option, $caller, $pkg)
	{
		...
		$this->store();
		if ($pkg != '') mosRedirect($caller."&pkg=$pkg", _FACILEFORMS_CONFIG_SAVED);
		mosRedirect($caller, _FACILEFORMS_CONFIG_SAVED);
	} // save
	...
	function getAllPackages(&$all)
	{
		$all = array();
		$rows = _ff_select(
			"select distinct binary package as package ".
	...
	function makePackage($option, $caller, $pkg)
	{
		...
		$lists['packages'] = _ff_select(
			"select *  from #__facileforms_packages order by binary id"
		);
	...
	function mkPackage($option, $caller, $pkg)
	{
		...
		$id          = $pkg;
		$name        = mosGetParam($_REQUEST, 'pkg_name', '');
		$title       = mosGetParam($_REQUEST, 'pkg_title', '');
		$version     = mosGetParam($_REQUEST, 'pkg_version', '');
		$created     = date('Y-m-d H:i:s');
		$author      = mosGetParam($_REQUEST, 'pkg_author', '');
		$email       = mosGetParam($_REQUEST, 'pkg_email', '');
		$url         = mosGetParam($_REQUEST, 'pkg_url', '');
		$description = mosGetParam($_REQUEST, 'pkg_description', '');
		$copyright   = mosGetParam($_REQUEST, 'pkg_copyright', '');
		...
		if ($id == '')
			$ids = mosGetParam($_REQUEST, 'scriptsel', array());
		else {
			$ids = array();
			$rows = _ff_select(
				"select id from #__facileforms_scripts ".
				"where package = binary '$id' ".
				"order by id"
			);
		...
		if (count($ids) > 0) {
			$ids = implode(',', $ids);
			$scripts = _ff_select(
				"select * from #__facileforms_scripts where id in ($ids) order by binary package, name, id"
			);
		...
		if ($id == '')
			$ids = mosGetParam($_REQUEST, 'piecesel', array());
		else {
			$ids = array();
			$rows = _ff_select(
		...
		if (count($ids) > 0) {
			$ids = implode(',', $ids);
			$pieces = _ff_select(
				"select * from #__facileforms_pieces where id in ($ids) order by binary package, name, id"
			);
		...
		if ($id == '')
			$ids = mosGetParam($_REQUEST, 'formsel', array());
		else {
			$ids = array();
			$rows = _ff_select(
		...
		if (count($ids) > 0) {
			$ids = implode(',', $ids);
			$forms = _ff_select(
				"select * from #__facileforms_forms where id in ($ids) order by binary package, ordering, id"
			);
		...
		if ($id == '')
			$ids = mosGetParam($_REQUEST, 'menusel', array());
		else {
			$ids = array();
			$rows = _ff_select(
				"select id from #__facileforms_compmenus ".
				"where package = binary '$id' and parent = 0 ".
				"order by id"
			);
			if (count($rows)) foreach ($rows as $row) $ids[] = $row->id;
		} // if
		if (count($ids) > 0) {
			$ids = implode(',', $ids);
			$menus = _ff_select(
				"select * from #__facileforms_compmenus where id in ($ids) order by binary package, ordering, id"
			);
	...
	function instLocalPackage($option, $caller, $pkg)
	{
		...
		$installfile = str_replace('{mospath}', $ff_mospath, mosGetParam($_REQUEST, 'installfile', ''));
		$errors = array();
		$errmode = 'log';
	...
	function instUploadPackage($option, $caller, $pkg)
	{
		...
		$uploadfile = mosGetParam($_FILES, 'uploadfile', '');
		if ($uploadfile['name']=='') {
			HTML_facileFormsConf::message(
				$option, $caller, $pkg, _FACILEFORMS_INSTALLER_NOUPLOADFILE, 'instpackage'
			);
			return;
		} // if
		$path = NULL;
	...
	function uploadFile( $filename, $userfile_name, &$path)
	{
		...
		$baseDir = mosPathName($ff_admpath.'/packages');
		if (!file_exists($baseDir))
			return _FACILEFORMS_INSTALLER_UPLOADNODIR;
		if (!is_writable($baseDir))
			return _FACILEFORMS_INSTALLER_UPLOADDIRNOTWRT;
		$path = $baseDir.$userfile_name;
		if (!move_uploaded_file($filename, $path))
			return _FACILEFORMS_INSTALLER_MOVEFAILED;
		$filemode = NULL;
		if (isset($mosConfig_fileperms)) {
			if ($mosConfig_fileperms!='')
				$filemode = octdec($mosConfig_fileperms);
		...
		if (isset($filemode))
			if (!@chmod($path, $filemode))
				return _FACILEFORMS_INSTALLER_CHMODFAILED;
		return '';
	} // uploadfile
	...
	function uninstPackages($option, $caller, $pkg, $ids)
	{
		...
		HTML_facileFormsConf::message(
			$option, $caller, $pkg,
			count($ids).' '._FACILEFORMS_INSTALLER_PKGSUNINST,
			'instpackage'
		);
	} // uninstPackages
