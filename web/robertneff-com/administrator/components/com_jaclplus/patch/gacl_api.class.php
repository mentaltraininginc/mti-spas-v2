...
class gacl_api extends gacl {
	...
	function get_group_data($group_id, $group_type = 'ARO') {
		...
		$this->db->setQuery( 'SELECT group_id, parent_id, name, lft, rgt, jaclplus FROM '. $table .' WHERE group_id='. $group_id );
	...
	\*======================================================================*/
	/* Hack for integration with JACLPlus Component */
	function add_group($name, $jaclplus='0', $parent_id=0, $group_type='ARO') {
		...
		if ($parent_id == 0) {
			...
			// <jaclplus> $rs = $this->db->query($query);
			$rs = $this->db->query();
		...
		$this->db->setQuery( 'INSERT INTO '. $table .' (group_id,parent_id,name,lft,rgt,jaclplus) VALUES ('. $insert_id .','. $parent_id .',\''. $this->db->getEscaped($name) .'\','. $parent_rgt .','. ($parent_rgt + 1) .','. $this->db->Quote($jaclplus) .')' );
		// <jaclplus> $rs = $this->db->query($query);
		$rs = $this->db->query();
	...
	\*======================================================================*/
	/* Hack for integration with JACLPlus Component */
	function edit_group($group_id, $jaclplus=NULL, $name=NULL, $parent_id=NULL, $group_type='ARO') {
		...
		$this->db->setQuery( 'UPDATE '. $table .' SET '. implode(',', $set) .' WHERE group_id='. $group_id );
		$rs = $this->db->query();
	...
	function _rebuild_tree($table, $group_id, $left = 1) {
		...
		$this->db->setQuery( "SELECT group_id FROM ". $table ." WHERE parent_id=". $group_id );
		$rows = $this->db->loadRowList();
		if ($this->db->getErrorNum()) {
		...
		$this->db->setQuery( "UPDATE ". $table ." SET lft=". $left .", rgt=". $right ." WHERE group_id=". $group_id);
		$rs = $this->db->query();
	...
	function del_group($group_id, $reparent_children=TRUE, $group_type='ARO') {
		...
		// <jaclplus> $group_details = $this->db->loadRow($query);
		...
		if ($parent_id == 0) {
			...
			// <jaclplus> $child_count = $this->db->loadResult($query);
