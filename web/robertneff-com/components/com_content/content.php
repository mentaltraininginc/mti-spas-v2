...
function showCategory( $id, $gid, &$access, $sectionid, $limit, $selected, $limitstart, $now  ) {
	...
	. ( $noauth ? "\n AND a.access IN ( $my->jaclplus )" : '' )
	. "\n AND $category->access IN ( $my->jaclplus )"
	. $and
	. "\n ORDER BY $orderby"
	;
	$database->setQuery( $query );
...
function show( $row, $params, $gid, &$access, $pop, $option, $ItemidCount=NULL ) {
	...
	if ( $access->canEdit ) {
		...
		if ( $row->id === null || !jaclplusInList( $row->access, $my->jaclplus ) ) {
			mosNotAuth();
			return;
		}
	} else {
		...
		//if ( $row->access > $gid ) {
		if ( !jaclplusInList( $row->access, $my->jaclplus ) ) {
			if ( $noauth ) {
				mosNotAuth();
				return;
			} else {
